...
针对每次查询只用一个索引的特点, 如何建议而外的索引来加速查询, 至关重要. 同时也要注意索引的开小, 删除不必要的索引.

### 存储模型
MongoDB通过内存mapping表把存储在磁盘上数据直接映射到内存, 每个`collection`存在单独的一个`namespace`表(记录元信息)和多个可扩展数据表.
![http://p0iombi30.bkt.clouddn.com/%E5%B1%8F%E5%B9%95%E5%BF%AB%E7%85%A7%202018-09-24%20%E4%B8%8B%E5%8D%883.58.50.png](http://p0iombi30.bkt.clouddn.com/%E5%B1%8F%E5%B9%95%E5%BF%AB%E7%85%A7%202018-09-24%20%E4%B8%8B%E5%8D%883.58.50.png)
数据结构使用双向链表, 每个`collection`的数据被组织在一段连续的磁盘空间链表中, mongo叫做`extents`. 这个`extent`会指向另一个`document`链表的头和尾(当然extent和extent之间也是连起来的). 每一个`document`也会和其相邻的`document`相连, 实际数据就以Bson的格式存储在上图中的`DocRecord`.

数据会直接在原来的地方修改. 为了预防数据修改超过了原来记录的分配的空间, 整条记录会被移动到一个更大的区域(附带一些填充占位的字节, 这些填充的字节当作缓存空间用于将来可以用于放置更大的空间, 那么具体有多少字节, 这就需要基于每个collection的修改统计值), 原来空出来的空间将会被释放, 有一张表会记录`free list`的大小.
基于上述移动空间的设计, 我们可以预见数据会变得片段化, 所以mongo需要周期性地运行`compact`命令, 会把数据移动到连续的空间内, 以提高IO性能.这个操作通常需要在线下进行, 副本集需要滚动更新.
...
{u'_id': ObjectId('5ba858bc91046605e83667dd'),
 ...
 u'lastname': u'He'}
```
mongo写的时候可以指定多种方式(policy)以代表写成功了, 如数据已经在磁盘持久化了; 或者数据已经在多个副本集传递(可以通过`getLasterror`拿到执行返回值). mongo是可以给副本集打`tag`, 所以可以基于`tag`制定写策略.

由于所有从mongo读到的数据都是过去的快照, 读到的数据可能已经被其它客户修改, 所以如果有一致性要求, 每次修改数据前可以先读一次验证数据, 然后再做修改.
mongo可以在修改前验证条件, 可以使用`findAndModify`api更新数据. 如果是`pymongo`那么`update`会帮做验证条件, 不需要额外做这一步.

mongo没有事务的概念(4.0以前), 对每个`document`的操作都是原子性的, 但是如果多个`document`被修改, 原子性是不能保证的.所以客户端需要做到多文件更新原子性.一个常见的技术是: 首先创建一个独立的文档(叫做transaction), 把所有需要更新的文档连接起来; 然后把所有需要更新的文档反链到transaction; 接着如下图做二段式提交完成事务.
![http://p0iombi30.bkt.clouddn.com/%E5%B1%8F%E5%B9%95%E5%BF%AB%E7%85%A7%202018-09-24%20%E4%B8%8B%E5%8D%885.31.48.png](http://p0iombi30.bkt.clouddn.com/%E5%B1%8F%E5%B9%95%E5%BF%AB%E7%85%A7%202018-09-24%20%E4%B8%8B%E5%8D%885.31.48.png)
...
![http://p0iombi30.bkt.clouddn.com/%E5%B1%8F%E5%B9%95%E5%BF%AB%E7%85%A7%202018-09-24%20%E4%B8%8B%E5%8D%885.39.34.png](http://p0iombi30.bkt.clouddn.com/%E5%B1%8F%E5%B9%95%E5%BF%AB%E7%85%A7%202018-09-24%20%E4%B8%8B%E5%8D%885.39.34.png)
如果住DB挂掉了, 那么集群将进行选举, 通过节点优先级,运行时间等条件从二级DB中选出新的主DB, 由于二级DB采用异步复制的形式, 这个新选出来的主DB可能不是最新的数据版本.
...
### 分片模型
想要做到写负载均衡, 需要使用MongoDB的`sharding`. 设置`sharding`配置, 一个`collection`可以通过`partition key`被分配到`chunks`内(一组key的范围), 多个`chunks`被分配到不同的`shards`, 每个`shards`都是副本集. 通过`sharding`, mongo可以存储无限量的数据, 这点在大数据应用场景十分重要.
![http://p0iombi30.bkt.clouddn.com/%E5%B1%8F%E5%B9%95%E5%BF%AB%E7%85%A7%202018-09-24%20%E4%B8%8B%E5%8D%888.03.33.png](http://p0iombi30.bkt.clouddn.com/%E5%B1%8F%E5%B9%95%E5%BF%AB%E7%85%A7%202018-09-24%20%E4%B8%8B%E5%8D%888.03.33.png)
在`sharding`模式下, 客户端需要连接到`MongoS`, 一个路由服务器把客户端的请求发送到合适的分片. 对于修改操作请求, 如果包含`partition key`, 那么基于`chunk/shard`映射表信息(从`config server`得到,并且本地缓存), 路由服务器可以找到对应chunk的节点服务器. 如果是读查询, 路由服务器会检测`partition key`是否包含在查询的部分条件中, 如果是那么就可以找到对应的主或二级`shard`; 但是也有可能查询条件不含有`partition key`, 那么路由会把请求发给每个`shard`.如果查询需要排序, 同时`partition key`在排序的条件中, 那么路由会一次按`partition key`排序`shards`; 如果不包含`partition key`, 路由服务器会把请发给每个`shard`, 然后做`merge-sort`.
![http://p0iombi30.bkt.clouddn.com/%E5%B1%8F%E5%B9%95%E5%BF%AB%E7%85%A7%202018-09-24%20%E4%B8%8B%E5%8D%888.08.12.png](http://p0iombi30.bkt.clouddn.com/%E5%B1%8F%E5%B9%95%E5%BF%AB%E7%85%A7%202018-09-24%20%E4%B8%8B%E5%8D%888.08.12.png)
另一方面, 路由服务器需要保障每个`shards`中的数据`chunks`大致一样多. 但不平衡的条件被检测到, 路由器会通知繁忙的`shard`触发迁移工作. 这个迁移工作是线上进行的, 数据迁移时进行多次delta检查, 直到最后数据迁移完毕, 目标服务器会通知`config server`新的`shard`信息, 通知源服务发送`StaleConfigException`给路由服务器, 让路由器重新读取配置信息. 后面的某个时间点, 源服务器旧的`shard`数据将会被删除. 如果在迁移的过程中发生高频率的更新操作, 那么迁移会停止,路由服务器会选择新的`chunk`去迁移数据.