...
## 前言
最近尝试用MongoDB替换Mysql， 由于Mysql的写能力限制， 32C128G的实例同时写入1M的数据CPU飙升， 更不用说如果是并发的情况。MongoDB通过`sharding`能够很好地解读写能力扩展的问题, 故作一篇网上大神博客的总结. MongoDB的架构大致如下图所示(v3.2): 
...
{u'_id_': {u'key': [(u'_id', 1)], u'ns': u'test_database.person', u'v': 2},
 u'lastname_1': {u'key': [(u'lastname', 1)],
  ...
  u'v': 2}}
```
索引创建完, 那么一个`collection`的多索引是如何配合来加速查询的呢? 
...
针对每次查询只用一个索引的特点, 如何创建主键以外的索引来加速查询至关重要! 同时也要注意索引的开小, 删除不必要的索引.

### 存储模型
MongoDB通过内存mapping表把存储在磁盘上数据直接映射到内存, 每个`collection`存在单独的一个`namespace`表(记录元信息)和多个可扩展(`extent`)数据文件.
...
数据结构使用双向链表, 每个`collection`的数据被组织在一段连续的磁盘空间链表中, mongo叫做`extents`. 这个`extent`会指向一组`document`链表的头和尾(当然extent和extent之间也是连起来的). 每一个`document`也会和其相邻的`document`相连, 实际数据就以Bson的格式存储在上图中的`DocRecord`.

任何修改数据会直接在原来的地方进行. 为了预防数据修改后大小超过了原来记录的分配的空间, 整条记录会被移动到一个更大的区域(附带一些填充占位的字节, 这些填充的字节当作缓存空间用于将来可以用于放置更大的空间, 那么具体填充多少字节, 这就需要每个collection有一个统计值, 专门记录修改的统计数据), 原来空出来的空间将会被释放, 有一张表会记录`free list`的大小.
...
### 分片模型
想要做到写负载均衡, 需要使用MongoDB的`sharding`. 设置`sharding`配置, 一个`collection`可以通过`partition key`被分割到多个`chunks`内(一组key的范围), 多个`chunks`被分配到不同的`shards`, 每个`shards`都是副本集. 通过`sharding`, mongo可以存储无限量的数据, 这点在大数据应用场景十分重要.
...
另一方面, 路由服务器需要保障每个`shards`中的数据`chunks`大致一样多. 当不平衡的条件被检测到, 路由器会通知繁忙的`shard`触发迁移工作. 这个迁移工作是线上进行的, 数据迁移时进行多次delta检查, 直到最后数据迁移完毕, 目标服务器会通知`config server`新的`shard`信息, 通知源服务发送`StaleConfigException`给路由服务器, 让路由器重新读取配置信息. 后面的某个时间点, 源服务器旧的`shard`数据将会被删除. 如果在迁移的过程中发生高频率的更新操作, 那么迁移会停止,路由服务器会选择新的`chunk`去迁移数据.