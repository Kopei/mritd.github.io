...
### 启动一个mysql实例
一般命令行启动mysql需要指定`basedir`, `datadir`, `user`, `log-error`, `pid-file`, `socket`等参数, `basedir`下面放置项目二进制可执行文件和库文件,pid文件和socket等文件. `datadir`目录下放置server log文件, innodb相关文件, `.ibd`数据和索引文件, `.frm`对象结构文件等.
```bash
...
幻读现象会在这种情况出现, A事务先执行`select * from t_1 where id=4`, 然后B事务执行`insert into t_1 values (4, 'Hanny')`, **如果此时隔离等级是`read committed`**,那么事务A再执行`select * from t_1 where id=4`就会读到这条数据, 出现所谓的**幻读**. 如果此时的隔离等级是`repeatable read`那么A事务再执行select将不会读到`id=4`的数据.
